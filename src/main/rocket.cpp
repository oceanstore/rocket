...
/* This is a bit ugly, but we don't want to include env_dpdk in the app, while
 * spdk_util, which we do need, uses some of the functions implemented there.
 * We're not actually using the functions that depend on those, so just define
 * them as no-ops to allow the app to link.
 */
extern "C" {
...
void spdk_free(void *buf) { assert(false); }
...
static float get_us_from_tsc(uint64_t tsc, uint64_t tsc_rate) {
  return ((float)tsc) * 1000 * 1000 / tsc_rate;
}

static const char *format_argname(const char *name) {
...
static void print_ptr(const char *arg_string, uint64_t arg) {
  printf("%-7.7s0x%-14jx ", format_argname(arg_string), arg);
}

static void print_uint64(const char *arg_string, uint64_t arg) {
  /*
...
static void print_string(const char *arg_string, const char *arg) {
...
static void print_object_id(const struct spdk_trace_tpoint *d,
                            struct spdk_trace_parser_entry *entry) {
  /* Set size to 128 and 256 bytes to make sure we can fit all the characters we
   * need */
  ...
  snprintf(ids, sizeof(ids), "%c%jd%s",
           g_flags->object[d->object_type].id_prefix, entry->object_index,
           related_id);
  printf("id:    %-17s", ids);
}

static void print_float(const char *arg_string, float arg) {
  printf("%-7s%-16.3f ", format_argname(arg_string), arg);
}

static void print_event(struct spdk_trace_parser_entry *entry,
                        uint64_t tsc_rate, uint64_t tsc_offset) {
...
static void print_event_json(struct spdk_trace_parser_entry *entry,
                             uint64_t tsc_rate, uint64_t tsc_offset) {
  struct spdk_trace_entry *e = entry->entry;
  const struct spdk_trace_tpoint *d;
  size_t i;
    ...
    if (d->new_object) {
      ...
      if (entry->object_index != UINT64_MAX) {
        spdk_json_write_named_string_fmt(g_json, "id", "%c%" PRIu64,
                                         object_type, entry->object_index);
  ...
  if (entry->related_index != UINT64_MAX) {
    spdk_json_write_named_string_fmt(
        g_json, "related", "%c%" PRIu64,
        g_flags->object[entry->related_type].id_prefix, entry->related_index);
  }
...
static void process_event(struct spdk_trace_parser_entry *e, uint64_t tsc_rate,
                          uint64_t tsc_offset) {
  if (g_json == NULL) {
...
static void print_tpoint_definitions(void) {
...
static int print_json(void *cb_ctx, const void *data, size_t size) {
  ssize_t rc;
...
int main(int argc, char **argv) {
  ...
  if (!file_name) {
    if (shm_id >= 0) {
      ...
      snprintf(shm_name, sizeof(shm_name), "/%s_trace.pid%d", app_name,
               shm_pid);
    }
    file_name = shm_name;
  }
  ...
  opts.lcore = lcore;
  opts.mode = app_name == NULL ? SPDK_TRACE_PARSER_MODE_FILE
